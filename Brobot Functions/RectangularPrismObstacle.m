function [vertex,face,faceNormals] = RectangularPrismObstacle(lower,upper,plotOptions,axis_h)
if nargin<4
        axis_h=gca;
    if nargin<3
...
if isfield(plotOptions,'plotFaces') && plotOptions.plotFaces
    %tcolor = [.2 .2 .8];
    %try delete(p); end
    %p = patch('Faces',face,'Vertices',vertex,'FaceVertexCData',tcolor,'FaceColor','flat','lineStyle','none');
end
